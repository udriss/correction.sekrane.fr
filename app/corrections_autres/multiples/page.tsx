...
export default function MultipleCorrectionsAutres() {
  ...
  const fetchOrCreateGenericActivity = async () => {
    try {
      ...
      if (response.ok) {
        ...
        router.push(`/corrections_autres/multiples?activityId=${activityData.id}`, { scroll: false });
      } else {
        const errorData = await response.json();
        ...
        if (response.status === 404) {
          ...
          // Mettre à jour l'URL sans rafraîchir la page
          router.push(`/corrections_autres/multiples?activityId=${newActivity.id}`, { scroll: false });
        } else {
          // Créer une instance d'Error et y attacher les détails
  ...
  const handleActivityChange = async (event: SelectChangeEvent<string>) => {
    ...
    if (newActivityId === "0") {
      ...
      try {
        ...
        router.push(`/corrections_autres/multiples?activityId=${newActivityId}`, { scroll: false });
      } catch (err) {
        console.error('Erreur:', err);
        setError(err instanceof Error ? err.message : String(err));
      } finally {
  ...
  const handleCreateGroup = async (createdCorrectionIds: string[]) => {
    ...
    try {
      ...
      setTimeout(() => {
        router.push(`/activities_autres/${activityId}/groups/${groupData.id}`);
      }, 2000);
    ...
    if (createdCorrectionIds.length > 0) {
      ...
      if (!selectedClassId && groupName.trim()) {
        ...
        setTimeout(() => {
          ...
          router.push(`/corrections_autres?classId=${selectedClassId}&highlight={highlightParam}&correctionId=${highlightParam}&activityId=${activityId !== "0" ? activityId : genericActivityId}`);
        }, 2000);
      } else {
        ...
        setTimeout(() => {
          const highlightParam = createdCorrectionIds.join(',');
          // Ajouter correctionId pour filtrer spécifiquement ces corrections
          router.push(`/corrections_autres?highlight=${highlightParam}&correctionId=${highlightParam}&activityId=${activityId !== "0" ? activityId : genericActivityId}`);
        }, 2000);
      }
    }
  };
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <FormControl fullWidth variant="outlined">
            ...
            <Button
              ...
              component={Link}
              href={`/activities_autres/${selectedActivityId !== "0" ? selectedActivityId : ''}`}
              fullWidth
