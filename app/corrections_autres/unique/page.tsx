...
export default function UniqueCorrection() {
  ...
  const handleActivityChange = async (event: SelectChangeEvent<string>) => {
    ...
    if (newActivityId === "0") {
      ...
      try {
        ...
        router.push(`/corrections_autres/unique?activityId=${newActivityId}`, { scroll: false });
      } catch (err) {
        console.error('Erreur:', err);
        setError(err instanceof Error ? err.message : String(err));
      } finally {
  ...
  const fetchOrCreateGenericActivity = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Update the URL without full refresh
        router.push(`/corrections_autres/unique?activityId=${activityData.id}`, { scroll: false });
      } else {
        const errorData = await response.json();
        ...
        if (response.status === 404) {
          ...
          // Update the URL without full refresh
          router.push(`/corrections_autres/unique?activityId=${newActivity.id}`, { scroll: false });
        } else {
          // Créer une instance d'Error et y attacher les détails
  ...
  const handleSuccessfulCreateCorrection = (createdCorrectionId: string) => {
    if (createdCorrectionId) {
      ...
      setTimeout(() => {
        router.push(`/corrections_autres/${createdCorrectionId}`);
      }, 2000);
    }
  };
