// filepath: /var/www/correction.sekrane.fr/app/api/corrections_autres/[id]/duplicate/route.ts
  ...
  try {
    ...
    return await withConnection(async (connection) => {
      ...
      const [activityResults] = await connection.query(
        `SELECT a.name as activity_name FROM corrections_autres c
      ...
      await createLogEntry({
        action_type: `DUPLICATE_CORRECTION_AUTRE_${mode}`,
        description: `Correction autre ${overwriteExisting ? 'écrasée' : 'dupliquée'} depuis #${correctionId} pour ${studentName} - ${activityName}`,
