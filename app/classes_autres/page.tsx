...
export default function ClassesPage() {
  ...
  const fetchClasses = async () => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ error: 'Erreur lors du chargement des classes' }));
      ...
      if (err instanceof Error) {
        setError(err.message || 'Impossible de charger les classes. Veuillez réessayer plus tard.');
      } else {
        ...
        <div className="container mx-auto px-4 py-2 flex justify-center">
          <div className="w-full max-w-4xl animate-slide-in">
            <Paper className="p-6 overflow-hidden relative" elevation={3}>
              <ErrorDisplay 
                error={error || "Erreur lors du chargement des classes"} 
                    ...
                    <Card className="h-full flex flex-col hover:shadow-lg transition-shadow border border-gray-100">
                      ...
                      <CardActions>
                        ...
                        <Button 
                          size="small" 
                          component={Link} 
                          href={`/classes_autres/${cls.id}/students`}
                          className="text-blue-700"
                        >
                          Gérer étudiants
                        </Button>
